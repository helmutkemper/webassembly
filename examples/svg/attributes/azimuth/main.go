// English:
//
// This example was taken from https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/azimuth
//
// Português:
//
// Este exemplo foi retirado do site https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/azimuth
//
	...
	s1 := factoryBrowser.NewTagSvg().ViewBox([]float64{0, 0, 440, 200}).Append(
		...
		factoryBrowser.NewTagSvgFilter().Id("distantLight1").Append(
			factoryBrowser.NewTagSvgFeDiffuseLighting().Append(
				factoryBrowser.NewTagSvgFeDistantLight().Azimuth(0),
			),
		),

		factoryBrowser.NewTagSvgFilter().Id("distantLight2").Append(
			factoryBrowser.NewTagSvgFeDiffuseLighting().Append(
				factoryBrowser.NewTagSvgFeDistantLight().Azimuth(240),
			),
		),
