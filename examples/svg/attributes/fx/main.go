// English:
//
// This example was taken from https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/fx
//
// Português:
//
// Este exemplo foi retirado do site https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/fx
//
...
func main() {
	...
	s1 := factoryBrowser.NewTagSvg().ViewBox([]float64{0, 0, 480, 200}).Append(
		factoryBrowser.NewTagSvgDefs().Append(
			...
			factoryBrowser.NewTagSvgRadialGradient().Id("gradient2").Cx(0.5).Cy(0.5).R(0.5).Fx(0.75).Fy(0.35).Fr(float32(0.05)).Append(
				factoryBrowser.NewTagSvgStop().Offset(float32(0.0)).StopColor(factoryColor.NewWhite()),
				factoryBrowser.NewTagSvgStop().Offset(float32(1.0)).StopColor(factoryColor.NewDarkseagreen()),
			),
		),
