// English:
//
// This example was taken from https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/k1
//
// Português:
//
// Este exemplo foi retirado do site https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/k1
//
...
func main() {
	...
	s1 := factoryBrowser.NewTagSvg().ViewBox([]float64{0, 0, 420, 200}).Append(
		...
		factoryBrowser.NewTagSvgFilter().Id("composite2").X(0).Y(0).Width(float32(1.0)).Height(float32(1.0)).Append(
			factoryBrowser.NewTagSvgFeComposite().In2(html.KSvgIn2SourceGraphic).Operator(html.KSvgOperatorFeCompositeArithmetic).K1(10).K2(0).K3(0).K4(0),
		),
