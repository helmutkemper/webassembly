// English:
//
// This example was taken from https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/repeatCount
//
// Português:
//
// Este exemplo foi retirado do site https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/repeatCount
//
...
func main() {
	...
	s1 := factoryBrowser.NewTagSvg().ViewBox([]float64{0, 0, 220, 150}).Append(
		factoryBrowser.NewTagSvgRect().X(0).Y(0).Width(100).Height(100).Append(
			factoryBrowser.NewTagSvgAnimate().AttributeName("y").From(0).To(50).Dur(1*time.Second).RepeatCount(5),
		),
