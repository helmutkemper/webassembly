...
func (el *Element) InitializeById( id string ) {
  ...
  el.selfElement = el.Document.Call("getElementById", id)
}
...
func (el *Canvas) CreateNewWith2DContext ( width, height float64 ) js.Value {
  cv := el.Document.Call("createElement", "canvas")
  cv.Set("width", width)
  cv.Set("height", height)
  return cv.Call("getContext", "2d")
}

func (el *Canvas) CreateNewWith3DContext ( width, height float64 ) js.Value {
  cv := el.Document.Call("createElement", "canvas")
  cv.Set("width", width)
  cv.Set("height", height)
  return cv.Call("getContext", "3d")
}
