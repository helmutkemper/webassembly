// English:
//
// This example was taken from https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/pointsAtZ
//
// Português:
//
// Este exemplo foi retirado do site https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/pointsAtZ
//
	...
	s1 := factoryBrowser.NewTagSvg().ViewBox([]float64{0, 0, 420, 200}).Append(
		factoryBrowser.NewTagSvgFilter().Id("lighting1").X(0).Y(0).Width(float32(1.0)).Height(float32(1.0)).Append(
			factoryBrowser.NewTagSvgFeDiffuseLighting().In(html.KSvgInSourceGraphic).Append(
				factoryBrowser.NewTagSvgFeSpotLight().X(60).Y(60).Z(50).PointsAtZ(0),
			),
		),

		factoryBrowser.NewTagSvgFilter().Id("lighting2").X(0).Y(0).Width(float32(1.0)).Height(float32(1.0)).Append(
			factoryBrowser.NewTagSvgFeDiffuseLighting().In(html.KSvgInSourceGraphic).Append(
				factoryBrowser.NewTagSvgFeSpotLight().X(60).Y(60).Z(50).PointsAtZ(80),
			),
		),
